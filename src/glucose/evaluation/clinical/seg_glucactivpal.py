...
#from glucose.evaluation.clinical.seg import plot_seg
...
MODEL_FILE: str = '20210106-105259_BasicRNN_glucose_MODEL.hdf5'
SCALER_FILE: str = '20210106-105259_BasicRNN_glucose_SCALER.pkl'
...
for horizon in ['15min', '30min', '60min']:
    concat_df = pd.concat([x[horizon] for id, x in raw_dict.items() \
                          if int(status.get(int(id), 3)) == 2])
    #concat_df = concat_df.append(pd.DataFrame.from_dict(
    #    {'time': [np.nan], 'real': [40], 'pred': [40]}
    #))
    print(concat_df.shape)
    concat_df['pred'] = concat_df['pred'] + ((concat_df['real'] - concat_df['pred']) * 0.11)
