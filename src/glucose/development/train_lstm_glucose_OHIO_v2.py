...
from glucose.utils.ohio import load_ohio_dataset
from glucose.models.LSTM import BasicLSTM
...
CONSTANTS = {
    ...
    'MODEL_DIR': 'E:\\Projecten\\Glucose voorspelling\\models\\glucose',
    'MODEL_FILE': '20201212-111238_BasicLSTM_glucose_MODEL.hdf5',
    'SCALER_FILE': '20201212-111238_BasicLSTM_glucose_SCALER.pkl',
    'LOOK_BACK': 6,
    'LOOK_FORWARD': [3, 6, 12],
    ...
    'EPOCHS': 200,
    'BATCH_SIZE': 128 ** 2,
    'LOSS_FUNC': {'15min': 'mse', '30min': 'mse', '60min': 'mse'},
    'LOSS_WEIGHT': {'15min': 1.0, '30min': 1.0, '60min': 1.0},
    'EARLYSTOPPER_PATIENCE': 10,

    'VERBOSE_LEVEL': 2,
    'NETWORK_CLASS': BasicLSTM,
    'DATA_TYPE': 'glucose_OHIO',
}
...
datasets = load_ohio_dataset(r'E:\Projecten\Glucose voorspelling\data\ohiot1dm\2020\train')
scaler = pickle.load(open(os.path.join(CONSTANTS['MODEL_DIR'],
                                       CONSTANTS['SCALER_FILE']), 'rb'))
model = load_model(os.path.join(CONSTANTS['MODEL_DIR'],
                                CONSTANTS['MODEL_FILE']))

logging.info('Dataset sizes: train [{}] - validation [{}] - ohio: [{}]'.format(
    len(train), len(validation), len(datasets),
))
...
for d in datasets:
    ...
    _id = d[0]
    _df = d[1]
    _df[CONSTANTS['DATA_COLS']] = scaler.transform(_df[CONSTANTS['DATA_COLS']])
    ...
    for i in range(len(_x)):
        xdata['ohio'].append(_x[i])
        ydata['ohio'].append(_y[i])

    xdata['ohio'] = np.array(xdata['ohio'])
    ydata['ohio'] = np.array(ydata['ohio'])
...
fit_data = xdata['ohio'][:, :, 1].reshape(-1, CONSTANTS['LOOK_BACK'], 1).astype(np.float64)
out_data = [transposed_ohioy[0].reshape(-1, 1),
            transposed_ohioy[1].reshape(-1, 1),
            transposed_ohioy[2].reshape(-1, 1)]
...
out_data_val = [transposed_validationy[0].reshape(-1, 1),
                ...
                transposed_validationy[2].reshape(-1, 1)]
...
# Actual fitting
model.fit(fit_data, [out_data[0], out_data[1], out_data[2]],
